...
def KK_General_PP(target_energies: npt.NDArray, 
    ...
    if imag_coefs.shape[0] != energies.shape[0]-1:
            raise ValueError(f"First axis of imaginary_spectrum must have one less row ({imag_coefs.shape[0]}) than energies ({energies.shape[0]})")
    if imag_coefs.shape[1] != orders.shape[0]:
            raise ValueError(f"Second axis of imaginary_spectrum must have the same number of columns ({imag_coefs.shape[1]}) as orders ({orders.shape[0]})")
    ...
    N = np.tile(orders[np.newaxis,np.newaxis,:],            (len(energies) - 1, len(target_energies),1)) #1D to 3D
    X = np.tile(energies[:,np.newaxis,np.newaxis],          (1, len(target_energies), len(orders))) # 1D to 3D
    E = np.tile(target_energies[np.newaxis, :, np.newaxis],   (len(energies) - 1, 1, len(orders))) #1D TO 3D
    # Calculate when evaluating energies are equal to the data energies, in a boolean array (1 or 0)
    poles = np.equal(X, 
        np.tile(target_energies[np.newaxis, :, np.newaxis],   (len(energies), 1, len(orders))
    ))
...
def KK_General_PP_inv(target_energies: npt.NDArray, 
                      energies: npt.NDArray, 
...
def improve_accuracy(energies: npt.ArrayLike,
    ...
    for i in range(max_iter):
        ...
        if np.any(energies[idx_extra] == energies[idx_extra-1]):
            idx_extra = np.delete(idx_extra, np.where(energies[idx_extra] == energies[idx_extra-1]))
        ...
        if np.sum(improved) == 0:
            ...
            idx_improved = idx_extra[improved] # insertion indexes for midpoints where improvements are made
            ...
            new_value_locs = np.insert(
                arr=    np.zeros(imag_coefs.shape[0], dtype=bool), # Copy existing midpoint list
                obj=    idx_extra[improved], # Add the locations where improvements were made 
                values= True # Insert True
            )
            new_midpoint_locs = np.where(new_value_locs)[0] # Locate the indexes of new midpoint values after insertions.
            ...
            for j in range(idx_improved.shape[0]-1, -1, -1):
                ...
                idx = idx_improved[j] - 1 # Get the midpoint index, move back one to get the coefficient index that defines that region
                # Duplicate the coefficients at the index
                imag_coefs = np.r_[imag_coefs[:idx], [imag_coefs[idx]], imag_coefs[idx:]] # Insert the new coefficients
            ...
            idx_extra = np.vstack((new_midpoint_locs,new_midpoint_locs+1)).T.flatten() 
            
    # Return values if the maximum number of iterations is reached
    return energies, real_asf

    
def improve_accuracy_inv(energies: npt.ArrayLike,
    ...
    for i in range(max_iter):
        ...
        if np.sum(improved) == 0:
            ...
            # Create new array of midpoint locations to evaluate.
            new_value_locs = np.insert(
                arr=    np.zeros(real_coefs.shape[0], dtype=bool), # Copy existing midpoint list
                obj=    idx_extra[improved], # Add the locations where improvements were made 
                values= True # Insert True
            )
            new_midpoint_locs = np.where(new_value_locs)[0] # Locate the indexes of new midpoint values after insertions.
            ...
            for j in range(idx_improved.shape[0]-1, -1, -1):
                ...
                idx = idx_improved[j] - 1 # Get the midpoint index, move back one to get the coefficient index that defines that region
                # Duplicate the coefficients at the index
                real_coefs = np.r_[real_coefs[:idx], [real_coefs[idx]], real_coefs[idx:]] # Insert the new coefficients
