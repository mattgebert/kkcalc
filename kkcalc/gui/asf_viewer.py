        ...
        for obj in self.scattering_objects:
            ...
            if isinstance(obj, asf_abstract):
                ...
                if isinstance(obj, asf_re) and graph_style in [GraphType.RE_IM_SEPARATE, GraphType.RE_IM_OVERLAY]:
                    self.ax1.plot(obj.energies, y_re, label=obj.name)
                elif isinstance(obj, asf_im) and graph_style in [GraphType.RE_IM_SEPARATE, GraphType.RE_IM_OVERLAY]:
                    self.ax2.plot(obj.energies, y_im, label=obj.name)
                elif isinstance(obj, asf_complex):
                    if graph_style in [GraphType.ABS_PHASE_SEPARATE, GraphType.ABS_PHASE_OVERLAY]:
                        self.ax1.plot(obj.energies, y_abs, label=obj.name)
                        self.ax2.plot(obj.energies, y_phase, label=obj.name)
...
if __name__ == "__main__":
    ...
    polystyrene_asp_im = asp_db_extended.from_asf_im(data_asf = polystyrene_data, data_stoich=PS_stoich, name="PS Ext.")
    polystyrene_asp_im2 = asp_db_extended.from_asf_im(data_asf = polystyrene_data, data_stoich=PS_stoich, merge_domain=[280, 320], name="PS Ext. Domain.")
    polystyrene_asf_re = polystyrene_asp_im.kk_transform()
    polystyrene_asf_re2 = polystyrene_asp_im2.kk_transform()
    
    objs: list[type[atomic_scattering]] = [polystyrene_data, polystyrene_asp_im, polystyrene_asp_im2, polystyrene_asf_re, polystyrene_asf_re2]
