...
class factor_complexity_dialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None, name: str|None = None):
        ...
        self.setWindowTitle("Select Complexity" if name is None else "Select Complexity: " + name)
        ...
        label = QtWidgets.QLabel("Select the complexity of the data" + (":" if name is None else f" for {name}:"))
...
class factor_dtype_dialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None, name: str|None = None):
        super().__init__(parent)
        self.setWindowTitle("Select datatype" if name is None else "Select datatype: " + name)
        self._layout = QtWidgets.QVBoxLayout()
        self.setLayout(self._layout)
        ...
        label = QtWidgets.QLabel("Select the datatype " + (":" if name is None else f" for {name}:"))
        self._layout.addWidget(label)
        blayout = QtWidgets.QHBoxLayout()
        self._layout.addLayout(blayout)
        for button in self.dtype_buttons:
...
class import_data_dialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None, path: str|None = None):
        ...
        [self.processor.setItemData(i, x.__doc__, role=QtCore.Qt.ItemDataRole.ToolTipRole)
        ...
        self.delimiter_edit.setToolTip("".join(["The delimiter used to separate the data.\n",
        ...
        self.viewer_table.setSelectionBehavior(QtWidgets.QAbstractItemView.SelectionBehavior.SelectColumns)
        ...
        self.result_rows_edit.setSizePolicy(QtWidgets.QSizePolicy.Policy.Minimum, QtWidgets.QSizePolicy.Policy.Ignored)
        self.result_rows_edit.setDisabled(True)
        self.result_cols_edit = QtWidgets.QLineEdit()
        self.result_cols_edit.setDisabled(True)
        self.result_cols_edit.setSizePolicy(QtWidgets.QSizePolicy.Policy.Minimum, QtWidgets.QSizePolicy.Policy.Ignored)
        self.show_graph_checkbox = QtWidgets.QCheckBox("Graph?")
        self.show_graph_checkbox.setToolTip("Show a preview of the data in a separate graph window.")
        ...
        self.x_column_select.setSizePolicy(QtWidgets.QSizePolicy.Policy.MinimumExpanding, QtWidgets.QSizePolicy.Policy.Ignored)
        ...
        self.y_column_select.setSizePolicy(QtWidgets.QSizePolicy.Policy.MinimumExpanding, QtWidgets.QSizePolicy.Policy.Ignored)
        ...
        self.result_cols_edit.setSizePolicy(QtWidgets.QSizePolicy.Policy.MinimumExpanding, QtWidgets.QSizePolicy.Policy.Ignored)
    ...
    def on_load(self):
        ...
        dtype: import_data_dialog.EnumProcessor = import_data_dialog.EnumProcessor(self.processor.currentIndex())
            ...
            with open(fname, "r") as f:
                ...
                match dtype:
                    ...
                    case import_data_dialog.EnumProcessor.PANDAS:
                        match fname.split(".")[-1]:
                            case "csv":
                                self.load_data = pd.read_csv(f, delimiter=delim, skiprows=skip_header_rows)
                            case "xls" | "xlsx":
                                self.load_data = pd.read_excel(f, skiprows=skip_header_rows)
                            case "h5" | "hdf5":
                                self.load_data = pd.read_hdf(f, skiprows=skip_header_rows)
                            case "xml":
                                self.load_data = pd.read_xml(f, skiprows=skip_header_rows)
                            case _:
                                # Default use table read
                                self.load_data = pd.read_table(f, delimiter=delim, skiprows=skip_header_rows)
                        print("Load: ", self.load_data)
                    case _:
        ...
        if self.load_data is not None:
            ...
            elif hasattr(self.load_data, "__len__") and not isinstance(self.load_data, str):
                self.result_rows_edit.setText(str(len(self.load_data)))
                if len(self.load_data) > 0:
                    if hasattr(self.load_data[0], "__len__") and not isinstance(self.load_data[0], str):
        ...
        if self.load_data is None:
            ...
            match self.load_dtype:
                case import_data_dialog.EnumProcessor.ASCII:
                    ...
                    for i, line in enumerate(self.load_data):
                        self.viewer_table.setItem(i, 0, QtWidgets.QTableWidgetItem(line))
                        
                case import_data_dialog.EnumProcessor.NUMPY:
                    # Display the data as a table
                    self.viewer_table.setRowCount(self.load_data.shape[0])
                    self.viewer_table.setColumnCount(self.load_data.shape[1])
                    self.viewer_table.setHorizontalHeaderLabels([str(i) for i in range(self.load_data.shape[1])])
                    for i, row in enumerate(self.load_data):
                        for j, value in enumerate(row):
                            self.viewer_table.setItem(i, j, QtWidgets.QTableWidgetItem(str(value)))
            ...
            if xcol != "":
                ...
                if xcol != self.__highlight_x and self.__highlight_x > -1:
                    for i in range(self.viewer_table.rowCount()):
                        self.viewer_table.item(i, self.__highlight_x).setBackground(self._default_brush)
                for i in range(self.viewer_table.rowCount()):
                    self.viewer_table.item(i, xcol).setBackground(QtGui.QBrush(QtGui.QColor(117, 250, 141, 128)))
                    # self.viewer_table.item(i, xcol).setForeground(QtGui.QBrush(QtGui.QColor(117, 250, 141, 128)))
                self.x_column_select.setStyleSheet("background-color: rgba(117, 250, 141, 128);")
            ...
            if ycol != "":
                ...
                if ycol != self.__highlight_y and self.__highlight_y > -1:
                    for i in range(self.viewer_table.rowCount()):
                        self.viewer_table.item(i, self.__highlight_y).setBackground(self._default_brush)
                for i in range(self.viewer_table.rowCount()):
                    self.viewer_table.item(i, ycol).setBackground(QtGui.QBrush(QtGui.QColor(159, 252, 253, 128)))
                    # self.viewer_table.item(i, ycol).setForeground(QtGui.QBrush(QtGui.QColor(159, 252, 253, 128)))
                self.y_column_select.setStyleSheet("background-color: rgba(159, 252, 253, 128);")
    ...
    def on_accept(self):
        ...
        try:
            ...
            return
        try:    
            rows, cols = int(self.result_rows_edit.text()), int(self.result_cols_edit.text())
        except:
    ...
    @property
    def selected_data(self) -> tuple[np.ndarray, np.ndarray]:
        """
        Collects the user selected data from the viewer.
        """
        X = self.load_data[:, int(self.x_column_select.text())]
        Y = self.load_data[:, int(self.y_column_select.text())]
        return X, Y
