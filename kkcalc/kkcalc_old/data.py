...
def merge_spectra(NearEdge_Data, ASF_E, ASF_Data, merge_points=None, add_background=False, fix_distortions=False, plotting_extras=False):
    ...
    scaled_NearEdge_Data = numpy.vstack(
        (
            (merge_points[0],asf_merge_values[0]),
            scaled_NearEdge_Data[NearEdge_merge_ind[0]:NearEdge_merge_ind[1]+1,:],
            (merge_points[1],asf_merge_values[1])
        )
    )
